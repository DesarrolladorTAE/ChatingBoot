...
const Conexiones: React.FC = () => {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-gray-800 mb-4">Conexiones</h1>

      {qrCodeState.loading && (
        <p className="text-center text-sm text-gray-500 mb-4">⌛ Generando QR...</p>
      )}
      {qrCodeState.qr && (
        <div className="bg-white p-4 rounded-xl shadow text-center mb-6">
          <img src={qrCodeState.qr} alt="QR Code" className="mx-auto w-48" />
          <p className="text-sm text-gray-600 mt-2">Escanea el código con WhatsApp</p>
        </div>
      )}
      {qrCodeState.error && (
        <p className="text-red-500 text-sm text-center mb-4">⚠️ {qrCodeState.error}</p>
      )}

      <div className="bg-white rounded-xl shadow overflow-hidden">
        {loading ? (
          <div className="py-12 text-center text-gray-500">Cargando conexiones...</div>
        ) : list.length === 0 ? (
            ...
            <tbody className="divide-y">
              {list.map((conn: Connection) => (
                <tr key={conn.connection_id}>
                  ...
                  <td className="px-6 py-3 text-center">{formatDate(conn.updated_at)}</td>
                  <td className="px-6 py-3 text-center">
                    {conn.is_team_default ? <FaCheckCircle className="text-emerald-500 mx-auto" /> : null}
                  </td>
                  <td className="px-6 py-3 text-right space-x-2">
                    <button onClick={() => handleEdit(conn)} className="text-yellow-600">
                      <FaEdit />
                    </button>
                    <button onClick={() => handleRemove(conn.id)} className="text-red-600">
                      <FaTrash />
                    </button>
                  </td>
                </tr>
      ...
      {error && (
        <p className="text-center text-red-500 mt-4 text-sm">⚠️ Error: {error}</p>
      )}

      <div className="flex justify-end mt-6">
        <button
          onClick={() => setShowModal(true)}
          className="bg-emerald-600 hover:bg-emerald-700 text-black px-5 py-2 rounded-lg transition font-medium shadow"
        >
          + Nueva Conexión
        </button>
      </div>
